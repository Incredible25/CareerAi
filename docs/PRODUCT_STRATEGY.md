...
**Formula history.** MVP v1 shipped with Goal Match and Opportunity Relevance dropped — neither was honestly computable yet — and the remaining six factors renormalized to 100%. **v1.1 (Phase 2, Module 3)** reintroduced Goal Match as deterministic keyword overlap between the user's free-text career goal and the career's own name/industry/interest tags (no NLP, no LLM — plain set intersection, so it only ever reflects words the user actually typed). Current weights: interest 22%, skill 22%, subject 14%, strength 14%, work-preference 10%, goal 10%, learning feasibility 8%. Opportunity Relevance stays dropped until the opportunity engine exists — there's no honest way to approximate it without one.
